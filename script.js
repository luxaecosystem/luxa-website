...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ===== WEB EXPLORER INTEGRATO (RISOLUZIONE CLIC BOTTONE) =====
    ...
    // Rendiamo la funzione globalmente accessibile affinché il pulsante HTML possa eseguirla
    window.triggerSearch = async function(overrideQuery) {
      ...
      try {
        if (isNumericBlock) {
          ...
          container.innerHTML = `
            <div style="background:rgba(0,0,0,0.6); border:1px solid #00FFCC; border-radius:16px; padding:20px; margin-top:16px;">
              ...
              <div style="font-size:12px; line-height:1.7; color:#cbd5e1;">
                ...
                <div><strong>Total Transactions:</strong> <span style="color:#FFD700; font-weight:bold;">${blk.data?.txs?.length || 0}</span></div>
                <div><strong>Proposer:</strong> <span style="font-family:monospace; color:#88BBFF; word-break:break-all;">${blk.header?.proposer_address}</span></div>
              </div>
            </div>
          `;
          return;
        ...
        container.innerHTML = `
          <div style="margin-top:16px; background:rgba(0,0,0,0.65); border:1px solid ${isNft ? '#FFD700' : '#00FFCC'}; border-radius:16px; padding:20px; box-shadow:0 0 25px ${isNft ? 'rgba(255,215,0,0.2)' : 'rgba(0,255,204,0.15)'};">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; border-bottom:1px solid rgba(255,255,255,0.08); padding-bottom:8px;">
            ...
            <button style="width:100%; margin-top:16px; background:transparent; border:1px solid ${isNft ? '#FFD700' : '#00FFCC'}; color:${isNft ? '#FFD700' : '#00FFCC'}; padding:10px; border-radius:8px; font-weight:bold; cursor:pointer;" onclick="navigator.clipboard.writeText('${query}'); alert('Transaction hash copied!');">
    ...
    // Gestione tasto Invio sulla barra di ricerca
    ...
    // Controllo parametri URL automatici (es. explorer.html?tx=...)
